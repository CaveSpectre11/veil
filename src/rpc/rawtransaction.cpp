...
static UniValue verifytxoutproof(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 999999)
        throw std::runtime_error(
...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("signrawtransaction")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "signrawtransaction is deprecated and will be fully removed in v0.18. "
            "To use signrawtransaction in v0.17, restart veild with -deprecatedrpc=signrawtransaction.\n"
...
UniValue decodepsbt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "decodepsbt \"psbt\"\n"
            "\nReturn a JSON object representing the serialized, base64-encoded partially signed Veil transaction.\n"
            ...
            "          \"hex\" : \"hex\",            (string) The hex\n"
            "          \"type\" : \"pubkeyhash\",    (string) The type, eg 'pubkeyhash'\n"
            "          \"address\" : \"address\"     (string) Veil address if there is one\n"
            "        }\n"
            "      },\n"
...
UniValue combinepsbt(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "combinepsbt [\"psbt\",...]\n"
            "\nCombine multiple partially signed Veil transactions into one transaction.\n"
